...
def bootstrap_mean(df1, df2, column, n=10000):
    ...
    for i in range(n):
        ...
        values_diff[i] = median_diff
    return values_diff

def percent(df1, df2, alpha):
    #faz o bootstrap e pega a diferença das medias
    values_diff = bootstrap_mean(df1, df2, 'TAXA_DE_PARTICIPACAO')

    #iintervalo de confianca
    lower_percentile = np.percentile(values_diff, (alpha / 2) * 100)
    upper_percentile = np.percentile(values_diff, (1 - alpha / 2) * 100)

    #inclui ou nao o zero
    return (lower_percentile > 0) or (upper_percentile < 0)
...
def t_obs(data):
  #O peso dos bebês médio é o mesmo.
  #Podemos explicar a variação de tais pesos devido a chance de que tal variação ocorre em uma seleção aleatória e uniforme de fumantes/não fumantes.
  #Grupo A: fumantes
  #Grupo B: não fumantes

  salario_charlotte = data[data['TEAM'] == 'Charlotte Hornets']['SALARY']
  salario_phoenix = data[data['TEAM'] == 'Phoenix Suns']['SALARY']

  obs_diff = salario_phoenix.mean() - salario_charlotte.mean()
  return obs_diff
...
def resposta(data):
  ...
  salario_charlotte = data[data['TEAM'] == 'Charlotte Hornets']['SALARY']
  salario_phoenix = data[data['TEAM'] == 'Phoenix Suns']['SALARY']
  #diferenca da media dos salarios dos dois times
  obs_diff = salario_charlotte.mean() - salario_phoenix.mean()
  ...
  for i in range(N):
      #sample
      perm_data = data.sample(frac=1, replace=False).reset_index(drop=True)
      perm_salario_charlotte = perm_data[perm_data['TEAM'] == 'Charlotte Hornets']['SALARY']
      perm_salario_phoenix = perm_data[perm_data['TEAM'] == 'Phoenix Suns']['SALARY']
      diff = perm_salario_charlotte.mean() - perm_salario_phoenix.mean()
      diferencas[i] = diff

  #probabilidade na diferenca observada
  p_value = (np.abs(diferencas) >= np.abs(obs_diff)).mean()
  #se maior que a significancia
  return p_value > 0.01
...
def t_obs2(data):
  ...
  # YOUR CODE HERE!!
  media_gsw = data.query('team_abbreviation == "GSW"')['player_height'].mean()
  media_cle = data.query('team_abbreviation == "CLE"')['player_height'].mean()
  #<ipython-input-106-c2cc2628fc37> in <cell line: 2>()
  #    1 result = t_obs2(df)
  #--> 2 assert_equal(round(result, 2), -1.43)
  estatistica_teste = media_cle - media_gsw
...
def critical_values(diffs):
  ...
  alpha = 0.05
  inferior = np.percentile(diffs, 100 * alpha / 2)
  superior = np.percentile(diffs, 100 * (1 - alpha / 2))
...
def p_value(t_obs, diffs):
  ...
  #contar quantas vezes a estatistida de teste na permutaçao é maior ou menor que a est de teste observada
  #a proporcao vai ser o p-valor
  if t_obs > 0:
      p_value = (diffs > t_obs).mean()
  else:
      p_value = (diffs < t_obs).mean()
  return p_value
...
def resposta2(diffs, t_obs):
  ...
  p_valuee = 0.2578 #esse valor de p-value é do outro exercicio, eu so peguei la
  if p_valuee <= alpha:
      return True  #rejeita
  else:
      return False  #nao rejeita
  return
