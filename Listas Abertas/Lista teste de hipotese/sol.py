...
def percent(df1, df2, alpha):
  ...
  for i in range(n):
    sample1 = df1[col].sample(size1, replace=True)
    sample2 = df2[col].sample(size2, replace=True)
    values_diff[i] = sample1.median() - sample2.median()

  alpha = alpha*100
  min = np.percentile(values_diff, alpha/2)
  max = np.percentile(values_diff, 100-(alpha/2))
  if min < 0 and max > 0:
    return False
  else:
    return True
...
def t_obs2(data):
  ...
  return data.query('team_abbreviation == "CLE"')['player_height'].mean() - data.query('team_abbreviation == "GSW"')['player_height'].mean()
